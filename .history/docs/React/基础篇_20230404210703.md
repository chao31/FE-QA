...
- useEffect(func,[])：仅第一次渲染的时候执行
- seEffect(func): 每次 `render` 后都会执行，传空类似于 `componentDidMount` 和 `componentDidUpdate` 一起的生命周期，在组件


#### 用 useEffect 模拟 componentWillUnmount 

 `useEffect` 依赖项`[]`，并返回一个函数 `fn`，组件销毁是执行 `fn`，等于 `componentWillUnmount`
