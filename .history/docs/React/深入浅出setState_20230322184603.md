...
`函数组件`多次调用 `+1` 操作，分别在不用时期：一个在 setTimeout `外`调用，另一个在 setTimeout `内`调用。`最后合并了，只调用了 1 次 +1`
...
类组件，多次调用 `+1` 操作，分别在不用时期，一个在 setTimeout `外`调用，另一个在 setTimeout `内`调用，`最后只调用了 1 次 +1`
