...
- 不熟悉 requestIdleCallback 可以点这里查看，这个方法很简单：它需要传入一个 callback，浏览器会在空闲时去调用这个 callback，然后给这个 callback 传入一个 IdleDeadline，IdleDeadline 会预估一个剩余闲置时间，我们可以通过还剩多少闲置时间去判断，是否足够去执行下一个单元任务。
