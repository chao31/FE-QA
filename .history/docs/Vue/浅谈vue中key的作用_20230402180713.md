...
注意：`vue` 判断两个节点`是否相同`时主要判断两者的 `key` 和`标签类型` (如 div) 等，因此如果不设置 `key`，它的值就是 `undefined`，则可能永远认为这是`两个相同节点`，只能去做更新操作，旧头 (1) 和新头 (1) 比较，是相同节点，更新，然后新、旧头指针往后移，再次对比旧头 (2) 和新头 (3) 比较，又是相同节点，更新 2-->3;指针再往后移，新节点指针已经到了结尾，对比结束，将剩余旧节点 3 删除。
