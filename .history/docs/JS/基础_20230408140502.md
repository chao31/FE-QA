...
2. `同域`下不同页面的 `localStorage` 可以共享吗？

不是`共享`，是`复制`。MDN 的说法：在该标签或窗口打开一个新页面时会复制顶级浏览会话的上下文作为新会话的上下文。

所以两种情况：

- 从 `A` 页面打开 (`<a>` 或 `window.open()`) 同源页面 `B`，会从 `A` 页面 复制一份 `seesionStorage` 存入 `B` 页面，但 `A`、`B` 修改互不影响。
- 单独起一个 `tab` 打开 `A` 页面，再单独起一个 `tab` 打开 `B`，即使同源，也会初始化两份 `seesionStorage`
